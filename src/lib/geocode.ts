...
export async function geocodeKorean(place: string): Promise<GeoResult | null> {
  ...
  const json = await res.json() as { documents: { y: string; x: string; address_name: string; place_name: string }[] }
  const doc = json.documents?.[0]
  if (!doc) return null

  return { lat: parseFloat(doc.y), lng: parseFloat(doc.x), address: doc.address_name, name: doc.place_name }
}
